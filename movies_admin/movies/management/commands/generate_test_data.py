...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # use list comprehensions because these are smaller & I want to sample from the entire set of these
        persons = [PersonFactory() for _ in range(NUM_PERSONS)]
        genres = [GenreFactory() for _ in range(NUM_GENRES)]

        self.stdout.write('Creating fake data relations...')
        for film in films:
